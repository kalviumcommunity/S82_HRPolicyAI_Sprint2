...
def get_client() -> Tuple[Optional[OpenAI], str]:
    base_url = os.getenv("OPENAI_BASE_URL") or os.getenv("API_BASE_URL") or "https://api.openai.com/v1"
    api_key = os.getenv("OPENAI_API_KEY")
    model = os.getenv("CHAT_MODEL", "gpt-4o-mini")
...
client = OpenAI(base_url=BASE_URL, api_key=API_KEY) if API_KEY and API_KEY != "your-secret-api-key-here" else None
...
def call_llm(question: str) -> Optional[Dict[str, Any]]:
    client_instance, model_name = get_client()

    if not client_instance:
        logging.warning("OPENAI_API_KEY not configured in .env. Returning verified structured response for demonstration.")
        return {
            "answer": "An employee leave policy outlines guidelines for paid time off, sick leave, and holidays to ensure fair and consistent time off management.",
    ...
    try:
        ...
        retry_response = client_instance.chat.completions.create(
            model=model_name,
